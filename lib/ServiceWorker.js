...
export class ServiceWorker {
    constructor(properties) {
        this.installedCallback = snackbar;
        this.local = false;
        this.messageCallback = snackbar;
        Object.assign(this, properties);
        this.register();
    }
    register() {
        // Make sure we are not running locally
        if (!this.local && location.hostname === "localhost")
            return;
        ...
        import("workbox-window").then(({ Workbox }) => {
            ...
            this.installedCallback &&
                workbox.addEventListener("installed", (event) => event.isUpdate && this.installedCallback());
            this.messageCallback &&
                workbox.addEventListener("message", (event) => event.data.meta === "workbox-broadcast-update" &&
                    this.messageCallback());
            workbox.register();
        });
    }
